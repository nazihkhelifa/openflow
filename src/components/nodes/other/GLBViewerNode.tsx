...
export function GLBViewerNode({ id, data, selected }: NodeProps<GLBViewerNodeType>) {
  ...
  const processFile = useCallback(
    (file: File) => {
      ...
      if (!name.endsWith(".glb") && !name.endsWith(".gltf")) {
        useToast.getState().show("Please upload a .GLB or .GLTF file", "warning");
        return;
      }
  ...
  const prevCapturedImageRef = useRef<string | null>(null);

  // Auto-resize to match captured image aspect ratio
  useEffect(() => {
    if (!nodeData.capturedImage || nodeData.capturedImage === prevCapturedImageRef.current) {
      prevCapturedImageRef.current = nodeData.capturedImage ?? null;
      return;
    }
    prevCapturedImageRef.current = nodeData.capturedImage;

    requestAnimationFrame(() => {
      getImageDimensions(nodeData.capturedImage!).then((dims) => {
        if (!dims) return;
  ...
  }, [id, nodeData.capturedImage, getNode, updateNode]);
  ...
  return (
    ...
    >
      ...
      {nodeData.glbUrl ? (
        ...
        <div
          className="nodrag nopan w-full flex-1 min-h-[150px] border border-dashed border-neutral-600 rounded flex flex-col items-center justify-center cursor-pointer hover:border-neutral-500 hover:bg-neutral-700/50 transition-colors"
          onClick={(e) => {
            e.stopPropagation();
