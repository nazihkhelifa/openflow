...
const mockUpdateImageLayer = vi.fn();
const mockReorderLayer = vi.fn();
...
const createMockAnnotationStore = (overrides = {}) => ({
  ...
  unifiedLayers: [
    { id: "layer-img-0", type: "image" as const, url: "data:image/png;base64,test", x: 0, y: 0, scaleX: 1, scaleY: 1 },
  ],
  ...
  updateImageLayer: mockUpdateImageLayer,
...
describe("AnnotationModal", () => {
  ...
  describe("Keyboard Shortcuts", () => {
    ...
    it("should call deleteLayer when Delete is pressed with shape selected", () => {
      mockAnnotationStore = createMockAnnotationStore({ selectedLayerId: "shape-1" });

      render(<AnnotationModal />);

      fireEvent.keyDown(window, { key: "Delete" });

      expect(mockDeleteLayer).toHaveBeenCalledWith("shape-1");
    });

    it("should call deleteLayer when Backspace is pressed with shape selected", () => {
      mockAnnotationStore = createMockAnnotationStore({ selectedLayerId: "shape-1" });
    ...
    it("should not delete when no shape is selected", () => {
      mockAnnotationStore = createMockAnnotationStore({ selectedLayerId: null });

      render(<AnnotationModal />);

      fireEvent.keyDown(window, { key: "Delete" });

      expect(mockDeleteLayer).not.toHaveBeenCalled();
    });
  });
