...
describe("getConnectedInputsPure", () => {
  ...
  it("should extract image from generateImage output", () => {
    const nodes = [
      makeNode("nb", "generateImage", { outputImage: "data:image/png;base64,nb" }),
      makeNode("out", "output"),
    ];
  ...
  it("should extract text from prompt source (outputText)", () => {
    const nodes = [
      makeNode("p", "prompt", { outputText: "generated text", prompt: "fallback" }),
      makeNode("gen", "generateImage"),
    ];
    const edges = [makeEdge("p", "gen", "text")];
    const result = getConnectedInputsPure("gen", nodes, edges);
  ...
  it("should return null when prompt has no outputText (top is always output)", () => {
    const nodes = [
      makeNode("p", "prompt", { outputText: null, prompt: "instructions only" }),
      makeNode("gen", "generateImage"),
    ];
    const edges = [makeEdge("p", "gen", "text")];
    const result = getConnectedInputsPure("gen", nodes, edges);
...
describe("validateWorkflowPure", () => {
  ...
  it("should detect missing text input on generateImage", () => {
    const nodes = [makeNode("gen", "generateImage")];
  ...
  it("should accept text-0 handle for generateImage", () => {
    const nodes = [makeNode("p", "prompt"), makeNode("gen", "generateImage")];
